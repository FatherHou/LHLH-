...
<form method="POST" action="SearchHou">
   <div id="searchcontainer">
			<input id="sAdmin" type="text" name="searchAdmin" class="form-control" placeholder="please input key words">
...
<c:when test="${fn:length(adminList)!=0 }">		
	<table id="ordertab" border="1" cellpadding="5" style="margin-left:5px;margin-right:30px;width:1000px;margin-left:50px;">
		...
		<c:forEach items="${adminList}" var="adminList" varStatus="loop">
			<tr>
			<td><font size="4">${adminList.admin_id}</font></td>
			<td><font size="4">${adminList.admin_name}</font></td>
			<td><font size="4">${adminList.admin_phone}</font></td>
			<td><font size="4">${adminList.actor}</font></td>
			</tr>
		</c:forEach>
	</table>
</c:when>
